...
const INPUT_PROPS_BLACKLIST = ['extraWidth', 'injectStyles', 'className', 'inputRef', 'inputStyle', 'minWidth', 'onAutosize', 'placeholderIsMinWidth']
...
class AutosizeInput extends Component {
	...
	renderStyles () {
		...
		return isIE && injectStyles ? (<style dangerouslySetInnerHTML={{ __html: `input#${this.state.inputId}::-ms-clear {display: none;}` }} />) : null
	}
	render () {
		...
		const inputStyle = { boxSizing: 'content-box', width: `${this.state.inputWidth}px`, ...this.props.inputStyle }
		...
		return (
			<div className={this.props.containerClassName} style={wrapperStyle}>
				{this.renderStyles()}
				<input {...inputProps} autoComplete='off' ref={this.inputRef} />
